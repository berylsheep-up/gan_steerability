...
def G_logistic_nonsaturating_steer(G, D, steer_z, steer_x, latent='z'):
    '''L_GAN = max( D(G(steer_z)) - D(steer_x))

    Arguments:
        steer_z {latent_vector} -- [z+aw]
        steer_x {image} -- [edit(x, a)]

    '''
    if latent == 'z':
        fake_images_out = G.get_output_for(steer_z, steer_x, is_training=True)
    else:
        # w
