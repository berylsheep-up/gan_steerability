...
     上下文将隐藏基本运行/训练循环的实现细节。
     它将正确设置一切环境，告诉是否应该停止运行，然后进行清理。
     用户应定期调用update，并使用should_stop确定是否应停止运行。

     参数：
         Submit_config：用于当前运行的SubmitConfig。
         config_module：用于当前运行的整个配置模块。
         max_epoch：用于更新的max_epoch变量的可选缓存值。
    """
